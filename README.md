...
- [`headless-bpm/SPEC.md`](headless-bpm/SPEC.md) + [`headless-bpm/TRACE.md`](headless-bpm/TRACE.md) + [`headless-bpm/ERD.md`](headless-bpm/ERD.md) — a large Core+Optional example (specification 0.14.0): a durable, BPMN 2.0-aligned business-process orchestration system with no built-in GUI, operated through CLI, REST and MCP. It uses `requirement_metadata`, Mermaid diagrams, composed verification and a paired TRACE.md, with a derived logical ERD companion. Implementation evidence in the trace is intentionally still `TBD` / planned.

## License

Every example in this repository (each `SPEC.md`, `TRACE.md` and `ERD.md`) is licensed under [Apache-2.0](LICENSE). Each file carries an `SPDX-License-Identifier: Apache-2.0` marker directly after its front matter.
