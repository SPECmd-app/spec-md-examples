...
The project is specification-first. `SPEC.md` defines the authoritative behavioral contract, while `TRACE.md` and `ERD.md` provide traceability and logical design context. The current specification release is **Headless BPM 0.18.0** and follows the **SPEC.md / SPECmd.app spec-driven development approach**.

> **Project status:** v0.18.0 is the current specification baseline. A reference TypeScript/PostgreSQL implementation now exists and is deployed; `TRACE.md`'s implementation/verification evidence is generated by that implementation's own test suite (490+ automated tests, all 141 acceptance scenarios passing) rather than left as `TBD` / `Planned`.
...
| `CHANGELOG_0.18.0.md` | Release-level summary of changes introduced in v0.18.0 (current) |
| `CHANGELOG_0.17.0.md` | Release-level summary of changes introduced in v0.17.0 (historical) |
| `CHANGELOG_0.16.0.md` | Release-level summary of changes introduced in v0.16.0 (historical) |
| `CHANGELOG_0.15.0.md` | Release-level summary of changes introduced in v0.15.0 (historical) |
| `CHANGELOG_0.14.0.md` | Release-level summary of changes introduced in v0.14.0 (historical) |
...
**Headless BPM specification:** `0.18.0`  
**SPEC.md Core:** `0.4.3`  
**SPEC.md Optional:** `0.4.3`  
**Status:** Draft

Version 0.14.0 added:
...
Versions 0.15.0 through 0.18.0 additively clarified and extended the specification, without weakening or removing any
previously defined MUST behavior or reusing a requirement ID:

- **0.15.0** — normative clarifications to WF-005 (deterministic Exclusive Gateway routing), HUM-001 (dynamic
  assignment), WF-001 (fully autonomous, agent-only processes with no User Task), ORG-008 (task organization-scope
  derivation), PAC-005 (a Process Access Profile as its own authorization source, with an explicit empty-profile-denies
  guarantee), and LOOP-010 (the causal message-hop carrier mechanism); the new optional ORG-011 Process-Organization
  association capability; promotion of TLIST-011/OPS-020 organization filtering from SHOULD to MUST; and a terminology
  alignment with BPMN 2.0 (`Inbound Event`/`Event Subscription`/`event.publish` renamed to `Inbound Message`/`Message
  Subscription`/`message.publish`, to stop colliding with BPMN's own Event Flow Node category).
- **0.16.0** — a second converging Parallel Gateway synchronization mode, `FIRST_ACTIVE_BRANCH` (WF-007 extended, new
  WF-016): the join completes on the first branch's arrival and cancels every other still-open branch through the
  existing cancellation cascade, as an ordinary audited outcome rather than an Incident.
- **0.17.0** — MCP hosted mode (new API-034): MCP MUST be exposable over a stateless, network-reachable transport
  (not only local stdio), authenticated and authorized identically to REST, so a conforming deployment can run on a
  horizontally-scaled or serverless host.
- **0.18.0** — a normative operator-visibility requirement (SYS-003 extended): a failed readiness/liveness check MUST
  record its underlying failure detail to a server-side, operator-only log channel, distinct from the check's own
  minimal, non-leaking public response.

These are additive to the administrative recovery and observability capabilities introduced in 0.13.x. See the
per-version `CHANGELOG_*.md` files for full detail.
...
Headless BPM 0.18.0 requires support for the following executable Flow Node types:
...
## What is intentionally not implemented in v0.18.0

The specification currently records several future/deferred topics and does **not** require them in v0.18.0, including:
...
- adopting FEEL as an additional Sequence Flow condition language (Open Issue #15) — a general-purpose Complex Gateway
  remains out of scope regardless; an AI-driven or otherwise complex routing decision is expected to be computed by a
  Service Task and exposed as a plain output variable, with the Gateway only ever evaluating a deterministic condition
  over that variable.
...
A reference implementation now exists; `TRACE.md` is regenerated directly from that implementation's real test-run
results rather than hand-maintained, so `Test: ACC-nnn`-style evidence reflects an acceptance scenario whose automated
test actually passed in the recorded run. Before implementation evidence exists for a given requirement, `TBD` and
`Planned` remain the correct values rather than invented evidence.
...
**Headless BPM 0.18.0** — durable BPMN orchestration without requiring a built-in GUI.
