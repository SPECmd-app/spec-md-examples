...
> **Note on this copy:** this file is an illustrative snapshot taken from
> [`SPECmd-app/specmd-cli`](https://github.com/SPECmd-app/specmd-cli), which
> implements the tool this document specifies. The four Normative Modules
> linked below are **not included in this examples repository** — their
> relative links will not resolve here. For the complete, resolvable
> Specification Set (including the modules and `TRACE.md`'s companion
> module traces), see the `specmd-cli` repository directly.
